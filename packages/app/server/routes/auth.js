...
import { findUserByEmail, findUserById, updateUserStatus, updateSuspendedToken, updateLastLogin, incrementLoginAttempts, validatePassword, findUserBySuspendedToken, resetLoginAttempts, } from '../services/user.js';
import { createRefreshToken, findRefreshToken, revokeRefreshToken, } from '../services/refresh-token.js';
...
import { loginBodySchema, refreshTokenSchema, unlockAccountSchema, } from '../schemas/auth.js';
export default async function authRoutes(fastify) {
    fastify.post('/auth/login', {
        ...
        const { email, password } = loginBodySchema.parse(JSON.parse(request.body));
            ...
            if (query && query.login_attempts >= 5 && user.status === 'active') {
                ...
                try {
                    await Promise.all([
                        ...
                        sendEmail(React.createElement(AccountLocked, {
                            token,
                        }), {
                ...
                reply.code(403).send({
                    error: 'You have reached maximum login attempts. An email has been sent to unlock your account.',
                });
                return;
            }
            reply.code(401).send({ error: 'Invalid credentials' });
    ...
    fastify.get('/auth/unlock-account/:token', {
        config: {
            rateLimit: {
                max: 3,
                timeWindow: '15 minutes',
